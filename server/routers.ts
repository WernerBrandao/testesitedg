...
import { mediaLibrary } from "../drizzle/schema.js";
import { generateAnalyticsPDF, type AnalyticsReportData } from "./pdf-generator.js";
...
export const appRouter = router({
  ...
  me: publicProcedure.query(async ({ ctx }) => {
    // ✅ Retornar usuário autenticado ou null
  ...
  login: publicProcedure
    ...
    if (!user || !user.passwordHash) {
      console.log('[4] ERRO: Usuário não encontrado ou sem hash');
      throw new TRPCError({
        code: 'UNAUTHORIZED',
    ...
    // 2) Validar senha
    console.log('[5] Iniciando verificação de senha...');
    ...
    if (!isValid) {
      console.log('[8] ERRO: Senha inválida');
      throw new TRPCError({
        code: 'UNAUTHORIZED',
        message: 'E-mail ou senha inválidos'
      });
    ...
    console.log('[9] Senha válida! Prosseguindo com login...');
  ...
  users: router({
    ...
    getById: adminProcedure.input(z.object({ id: z.number() }).passthrough()).query(async ({ input }) => db.getUserById(input.id)),
  ...
  categories: router({
    ...
    getBySlug: publicProcedure.input(z.object({ slug: z.string() }).passthrough()).query(async ({ input }) => db.getCategoryBySlug(input.slug)),
    create: adminProcedure.input(z.object({
      name: z.string().min(1),
      slug: z.string().optional(),
  ...
  pages: router({
    ...
    getBySlug: publicProcedure.input(z.object({ slug: z.string() }).passthrough()).query(async ({ input }) => db.getPageBySlug(input.slug)),
    getById: publicProcedure.input(z.object({ id: z.number() }).passthrough()).query(async ({ input }) => db.getPageById(input.id)),
    create: adminProcedure.input(z.object({
      title: z.string().min(1),
      slug: z.string().optional(),
      content: z.string().min(1),
  ...
  banners: router({
    ...
    .query(async ({ input }) => {
      ...
      const banners = await db.listBanners(visibility);
  ...
  videos: router({
    ...
    .query(async ({ input }) => {
      ...
      const videos = await db.listVideos(visibility);
      
      // Filtrar apenas ativos
      return (videos || []).filter((v: any) => v.isActive);
    }),
    create: editorProcedure.input(z.object({
      title: z.string().min(1),
      description: z.string().optional(),
  ...
  media: router({
    getLibrary: publicProcedure.input(z.object({ limit: z.number().default(50), offset: z.number().default(0) }).passthrough()).query(async ({ input }) => db.getMediaLibrary(input.limit, input.offset)),
    getByType: publicProcedure.input(z.object({ fileType: z.enum(['image', 'video']), limit: z.number().default(50), offset: z.number().default(0) }).passthrough()).query(async ({ input }) => db.getMediaByType(input.fileType, input.limit, input.offset)),
    getById: publicProcedure.input(z.object({ id: z.number() }).passthrough()).query(async ({ input }) => db.getMediaById(input.id)),
  ...
  services: router({
    ...
    .query(async ({ input }) => {
      ...
      // Chamar db.listServices com filtro de visibilidade
      const services = await db.listServices(true, visibility);
      
      return services || [];
    }),
    getById: publicProcedure.input(z.object({ id: z.number() }).passthrough()).query(async ({ input }) => db.getServiceById(input.id)),
    create: adminProcedure.input(z.object({
      name: z.string().min(1),
      icon: z.string().min(1),
      link: z.string().url(),
  ...
  documents: router({
    ...
    listByCategory: publicProcedure.input(z.object({ categoryId: z.number() }).passthrough()).query(async ({ input }) => db.listDocumentsByCategory(input.categoryId)),
    ...
    search: publicProcedure.input(z.object({ query: z.string().min(1) }).passthrough()).query(async ({ input }) => db.searchDocuments(input.query)),
    ...
    getFeaturedByCategory: publicProcedure.input(z.object({ categoryId: z.number().optional(), limit: z.number().default(3), offset: z.number().default(0) }).passthrough()).query(async ({ input }) => db.getFeaturedDocumentsByCategory(input.categoryId, input.limit, input.offset)),
  ...
  documentVersions: router({
    list: publicProcedure.input(z.object({ documentId: z.number() }).passthrough()).query(async ({ input }) => db.getDocumentVersions(input.documentId)),
    create: protectedProcedure.input(z.object({
      documentId: z.number(),
      fileUrl: z.string().min(1),
      fileKey: z.string().min(1),
  ...
  pageBlocks: router({
    list: publicProcedure.input(z.object({ pageId: z.number() }).passthrough()).query(async ({ input }) => db.getPageBlocks(input.pageId)),
    create: protectedProcedure.input(z.object({
      pageId: z.number(),
  ...
  pageBlockItems: router({
    list: publicProcedure.input(z.object({ blockId: z.number() }).passthrough()).query(async ({ input }) => db.getPageBlockItems(input.blockId)),
    create: protectedProcedure.input(z.object({
      blockId: z.number(),
  ...
  imagesBank: router({
    list: publicProcedure.input(z.object({ limit: z.number().default(100), offset: z.number().default(0) }).passthrough()).query(async ({ input }) => db.getImagesBank(input.limit, input.offset)),
