...
export async function listPosts(opts: { status?: string; categoryId?: number; limit?: number; offset?: number; search?: string; featured?: boolean; visibility?: string } = {}) {
  ...
  // 🔥 Busca corrigida para compatibilidade total MySQL 8.0 + utf8mb4_0900
  if (opts.search) {
  ...
  // 🔥 Normalização para evitar erros no frontend
  items.forEach(p => {
    p.excerpt = p.excerpt ?? "";
    p.content = p.content ?? "";
...
//}
export async function listBanners(activeOnly = false, visibility?: string) {
  const db = await getDb();
  if (!db) return [];
...
//}
export async function listVideos(activeOnly = false, visibility?: string) {
  const db = await getDb();
  if (!db) return [];
...
// Função createMenuItem - CORRIGIDA
export async function createMenuItem(data: InsertMenuItem) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  
  // ✅ FIX: Converter null em undefined para evitar erro SQL
