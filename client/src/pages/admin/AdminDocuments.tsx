...
// 🔹 IMPORTS DO DND-KIT PARA A ÁREA DE DESTAQUES
import {
  DndContext,
  closestCenter,
  PointerSensor,
...
export default function AdminDocuments() {
  ...
  const [visibility, setVisibility] =
    useState<"site" | "intranet" | "both">("site");

  const utils = trpc.useUtils();
  const { data: documents, isLoading: documentsLoading } =
    trpc.documents.list.useQuery();
  const { data: categories } =
    trpc.documentCategories.list.useQuery();
  ...
  const createCategoryMutation =
    trpc.documentCategories.create.useMutation({
      onSuccess: () => {
  ...
  const deleteCategoryMutation =
    trpc.documentCategories.delete.useMutation({
      onSuccess: () => {
        utils.documentCategories.list.invalidate();
  ...
  const updateDocumentNameMutation =
    trpc.documents.updateName.useMutation({
      onSuccess: () => {
        utils.documents.list.invalidate();
  ...
  const documentsByCategory =
    filteredDocuments?.reduce((acc, item: any) => {
  ...
  // -------------------------------------------------------------------
  // ⚡ DND-KIT — COMPONENTE SORTABLE PARA DOCUMENTOS EM DESTAQUE
  // -------------------------------------------------------------------

  function SortableFeaturedItem({ doc, index }: any) {
    const {
      attributes,
      listeners,
      setNodeRef,
    ...
    return (
      <div
        ref={setNodeRef}
        style={style}
        className={`flex items-center gap-3 p-3 bg-gray-50 rounded-lg border cursor-grab 
          transition-colors ${
            isDragging
  ...
  function handleFeaturedDragEnd(event: any) {
    ...
    const oldIndex = featured.findIndex(
      (i: any) => i.id.toString() === active.id
    );
    const newIndex = featured.findIndex(
    ...
    const payload = reordered.map((d: any, i: number) => ({
      id: d.id,
      sortOrder: i,
    }));

    reorderMutation.mutate({ orders: payload });
  }
  ...
  return (
    <div>
        ...
        <div className="flex gap-2">
          <Link href="/admin/documentos/estatisticas">
            <Button
              variant="outline"
              style={{
                borderColor: "var(--degase-blue-dark)",
      ...
      {/* ------------------------------------------------------------ */}
      {/* 🔥 ÁREA DE DOCUMENTOS EM DESTAQUE — COM DND-KIT */}
      {/* ------------------------------------------------------------ */}
      ...
      <div className="bg-white rounded-lg border p-5 mb-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Button
            ...
            size="sm"
            onClick={() =>
              setShowFeaturedReorder(!showFeaturedReorder)
            }
          >
        ...
        {showFeaturedReorder && documents && (
          <div className="space-y-2 mb-4">
            <p className="text-sm text-gray-600 mb-3">
              Arraste os documentos para reordenar. Clique em
            ...
            {documents.filter((d: any) => d.isFeatured)
              .length === 0 && (
      ...
      {/* ------------------------------------------------------------ */}
      {/* MODAL DE EDITAR NOME DO DOCUMENTO */}
      {/* ------------------------------------------------------------ */}
      ...
      {/* ------------------------------------------------------------ */}
      {/* PAINEL PRINCIPAL DE DOCUMENTOS POR CATEGORIA */}
      {/* ------------------------------------------------------------ */}
      ...
      {documentsLoading ? (
        ...
        <div className="space-y-6">
          {Object.values(documentsByCategory).map(
            (group: any) => (
              <div
